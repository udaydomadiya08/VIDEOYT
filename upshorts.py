...
Write a compelling, SEO-optimized YouTube video description for the topic: "{topic}". The description must:

- Start with a powerful hook that grabs immediate attention
- Seamlessly include high-ranking, viral SEO keywords related to the topic
- Clearly and engagingly explain what the viewer will see in the video
- Motivate the audience to like, comment, and subscribe in a natural, persuasive tone
- Use a single, clean paragraph format (no lists, no titles, no line breaks)
- End with exactly 15 of the strongest-performing YouTube hashtags

🚨 Hashtag Rules:
- Must be **1 word** only
- Must be **ultra-high volume**, **high CTR**, and **SEO-optimized**
- Must relate to abandoned city, urbex, survival, ghost town, solo challenge
...
def generate_youtube_tags(topic):
    ...
    You are a YouTube SEO expert with access to real-time vidIQ Pro and TubeBuddy Boost data. For the video titled "{topic}", generate exactly 20 top-performing YouTube tags.

    Rules:
    - Tags must be **1 word** max only
    - Must be ultra-high volume, CTR, and SEO strength
    - No generic or medium-tier terms
    - Must match niches: abandoned city, urbex, survival, ghost town, solo challenge
    - Output only **comma-separated list**, no quotes, no newlines, under 500 characters
...
def upload_video(file_path, topic):
    ...
    final_tags = generate_youtube_tags(topic)
